...
from a lab written by the faculty and TAs of UCLA Statistics.  Alan Arnholt made additional modifications to the OpenIntro lab.
</div>
...
<div id="instructions">
**Directions:**  Use complete sentences answer all questions and exercises.  Submit your knitted html file to [crowdgrader](http://crowdgrader.com).
</div>
...
<div id="answer">
Type your answer for Question 1 between the answer tagss and delete this comment.
...
```{r label = "rvatbats", fig.cap = "Scatterplot of `runs` versus `at_bats` with superimposed regression line"}
...
<div id="exercise">
**Exercise 1**: If you knew a team's `at_bats`, would you be comfortable using 
a linear model to predict their number of runs?
</div>

<div id="answer">
Type your answer for Exercise 1 between the answer tags and delete this comment.
...
markdown document. Running the function also requires that the `mlb11` dataset be loaded 
in your environment.
</div>
...
also useful to be able to describe the relationship between two numerical variables, 
such as `runs` and `at_bats`.

3. Looking at your plot from the previous exercise, which of the following best describes the relationship between these two variables? 
<ol>
...
```{r label = "plotss-atbats-runs", eval=FALSE}
plot_ss(x = at_bats, y = runs, data = mlb11)
```

After running this command, you will be prompted to click two points on the plot 
...
of the 30 observations. Recall that the residuals are the differences between the
...
```{r label = "plotss-atbats-runs-squares", eval=FALSE}
...
<div id="exercise">
**Exercise 2**: Using `plot_ss`, choose a line that does a good job of minimizing 
the sum of squares.  Run the function several times.  Report your smallest sum 
of squares.
</div>

<div id="answer">
Type your answer for Exercise 2 between the answer tags and delete this comment.
...
the summary function as shown below and in Table \@ref(tab:m1tab).
...
Let us consider this output piece by piece. First, the formula used to describe 
...
\[
  \widehat{\text{runs}} = `r round(summary(m1)$coef[1, 1],4)` + `r round(summary(m1)$coef[2, 1],4)` \times \text{at_bats}
\]
...
explanatory variable. For this model, `r round(summary(m1)$r.squared*100, 2)`% of the variability in runs is 
explained by `at-bats`.

```{r, label = "m1tab", echo = FALSE}
# htmlTable::htmlTable(round(summary(m1)$coef, 4), css.cell = "padding-left: 1em; padding-right: 1em;", caption = "Coefficients for `runs` regressed on `at_bats`", align = "rrrr")
                                                                                                                 
knitr::kable(round(summary(m1)$coef, 4), caption = "Coefficients for `runs` regressed on `at_bats`")
```
...
Here, we are adding a layer on top of our plot. `stat_smooth` creates
the line by fitting a linear model. It can also show us the standard error, `se`, associated with our line, but we will suppress that for now.
...
This code first filters for rows where the `at_bats` equals 5579. Then, it shows the 
value of the `runs` variable for that condition.

<div id="exercise">
**Exercise 3**: If a team manager saw the least squares regression line and 
not the actual data, how many runs would he or she predict for a team with 
5,579 at-bats? Is this an overestimate or an underestimate of the actual value, and by how much?
</div>
...
```{r label = "residuals", fig.cap = "Residuals versus fitted values for model `m1`"}
...
Notice here that our model object `m1` can serve as a data set because stored within it are the fitted values ($\hat{y}$) and the residuals. Also note that we're getting fancy with the code. After creating the scatterplot on the first layer (first line of code), we overlay a horizontal dashed line at $y = 0$ (to help us check whether residuals are distributed around 0), and we adjust the axis labels to be more informative.
...
**Nearly normal residuals**: To check this condition, we can look at a density plot or a normal probability plot of the residuals.

```{r label = "dens-qq", fig.width = 9}
p1 <- ggplot(data = m1, aes(x = .resid)) +
      geom_density(fill = "lightblue") +
      labs(x = "Residuals") + 
      theme_bw()
p2 <- ggplot(data = m1, aes(sample = .resid)) +
      stat_qq() +
      theme_bw()
gridExtra::grid.arrange(p1, p2, ncol = 2)
```

Note that the syntax for making a normal probability plot is a bit different from what you have seen too date. We set `sample` equal to the residuals instead of `x`, and we set a statistical method `qq`, which stands for "quantile-quantile," another name commonly used for normal probability plots.
...
8. Based on Figure \@ref(fig:residuals), the constant variability condition appears to be met. 
<ol> 
<li> True </li>
<li> False </li>
</ol>
...
<div id="exercise">
**Exercise 4**: Choose another one of the seven traditional variables from 
...
```{r, label = "runsvselse", fig.cap = "`runs` versus `hits`"}
...
<div id="exercise">
**Exercise 5**: How does this relationship compare to the relationship between `runs` and `at_bats`? Use the R$^2$ values from the two model summaries to compare. Does your variable seem to predict `runs` better than `at_bats`? How can you tell?
</div>
...
```{r label = "runs-vs-others", fig.width = 10}
...
```{r label = "runs-vs-new-vars", fig.width = 13.5}
...
<div id="exercise">
**Exercise 6**: Check the model diagnostics for the regression model with the 
variable you decided was the best predictor for runs.
</div>

```{r label = "diag", fig.width = 13.5}
...
<div id="answer">
Type your answer for Exercise 6 between the answer tags and delete this comment.
