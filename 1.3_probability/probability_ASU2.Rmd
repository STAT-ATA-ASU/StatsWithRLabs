...
next. However, @gilovich_hot_1985 collected evidence
...
(1) to think about the effects of independent and dependent events, (2) to learn 
...
<div id="answer">
Type your answer to Question 2 here and delete this sentence.


</div>

Counting streak lengths manually for all 133 shots would be tedious, so we will
use the custom function `calc_streak` to calculate them, and store the results
in a data frame called `kobe_streak` as the `length` variable.  The following code defines the function `calc_streak`.  Read through the code and provide a
...
<div id="answer">
Provide appropriate comments below and delete this sentence.
...
to support the belief that he had hot hands? To what can we compare them?
...
<div id="exercise">
**Exercise 1**: In your simulation of flipping the unfair coin 100 times, how many flips came up heads?
</div>

```{r, label = "simanswer"}
# Type your code for Exercise 1 here, and Knit.
...
If you want to learn more about `sample`, type `?sample`.
...
<div id="exercise">
**Exercise 2**: What change needs to be made to the `sample` function so that it reflects a shooting percentage of 45%? Make this adjustment, then run a simulation to sample 133 shots. Assign the output of this simulation to a new object called `sim_basket`.
</div>
...
The argument `prob = c(0.45, 0.55)` should be used.
</div>
   
Note that we have named the new vector `sim_basket`, the same name we assigned to
the previous vector reflecting a shooting percentage of 50%. In this situation, 
R overwrites the old object with the new one, so always make sure that you do not
...
<div id="exercise">
**Exercise 4**: Make a histogram of the distribution of simulated streak lengths of the independent shooter. What is the typical streak length for this simulated independent shooter with a 45% shooting percentage? How long is the player's longest streak of baskets in 133 shots?
</div>

```{r label = "plot-sim-streaks"}
# Type your code for the Exercise 4 here, and Knit.


```

```{r, label = "kobegraph", echo = FALSE}
# Create Kobe Graph
...
```{r, label = "extg", echo = FALSE, fig.height = 8.5, fig.width = 8.5}
sim_streak  <- calc_streak(sample(shot_outcomes, size = 133, replace = TRUE, prob = c(0.45, 0.55)))
p1 <- ggplot(data = sim_streak, aes(x = streak_length)) + 
  geom_histogram(binwidth = 1, fill = "green", color = "black") + 
  theme_bw() + 
  labs(x = "Streak length", title = "Independent Shooter") + 
  xlim(-1, 10) + 
  ylim(0, 40)
sim_streak  <- calc_streak(sample(shot_outcomes, size = 133, replace = TRUE, prob = c(0.45, 0.55)))
p2 <- ggplot(data = sim_streak, aes(x = streak_length)) + 
  geom_histogram(binwidth = 1, fill = "green", color = "black") + 
  theme_bw() + 
  labs(x = "Streak length", title = "Independent Shooter") + 
  xlim(-1, 10) + 
  ylim(0, 40)
sim_streak  <- calc_streak(sample(shot_outcomes, size = 133, replace = TRUE, prob = c(0.45, 0.55)))
p3 <- ggplot(data = sim_streak, aes(x = streak_length)) + 
  geom_histogram(binwidth = 1, fill = "green", color = "black") + 
  theme_bw() + 
  labs(x = "Streak length", title = "Independent Shooter") + 
  xlim(-1, 10) + 
  ylim(0, 40)
sim_streak  <- calc_streak(sample(shot_outcomes, size = 133, replace = TRUE, prob = c(0.45, 0.55)))
p4 <- ggplot(data = sim_streak, aes(x = streak_length)) + 
  geom_histogram(binwidth = 1, fill = "green", color = "black") + 
  theme_bw() + 
  labs(x = "Streak length", title = "Independent Shooter") + 
  xlim(-1, 10) + 
  ylim(0, 40)
sim_streak  <- calc_streak(sample(shot_outcomes, size = 133, replace = TRUE, prob = c(0.45, 0.55)))
p5 <- ggplot(data = sim_streak, aes(x = streak_length)) + 
  geom_histogram(binwidth = 1, fill = "green", color = "black") + 
  theme_bw() + 
  labs(x = "Streak length", title = "Independent Shooter") + 
  xlim(-1, 10) + 
  ylim(0, 40)
sim_streak  <- calc_streak(sample(shot_outcomes, size = 133, replace = TRUE, prob = c(0.45, 0.55)))
p6 <- ggplot(data = sim_streak, aes(x = streak_length)) + 
  geom_histogram(binwidth = 1, fill = "green", color = "black") + 
  theme_bw() + 
  labs(x = "Streak length", title = "Independent Shooter") + 
  xlim(-1, 10) + 
  ylim(0, 40)
sim_streak  <- calc_streak(sample(shot_outcomes, size = 133, replace = TRUE, prob = c(0.45, 0.55)))
p7 <- ggplot(data = sim_streak, aes(x = streak_length)) + 
  geom_histogram(binwidth = 1, fill = "green", color = "black") + 
  theme_bw() + 
  labs(x = "Streak length", title = "Independent Shooter") + 
  xlim(-1, 10) + 
  ylim(0, 40)
sim_streak  <- calc_streak(sample(shot_outcomes, size = 133, replace = TRUE, prob = c(0.45, 0.55)))
p8 <- ggplot(data = sim_streak, aes(x = streak_length)) + 
  geom_histogram(binwidth = 1, fill = "green", color = "black") + 
  theme_bw() + 
  labs(x = "Streak length", title = "Independent Shooter") + 
  xlim(-1, 10) + 
  ylim(0, 40)
p9 <- ggplot(data = kobe_streak, aes(x = streak_length)) +
  geom_histogram(binwidth = 1, fill = "darkgreen", color = "black") + 
  theme_bw() + 
  labs(x = "Streak length", title = "Kobe Bryant") + 
  xlim(-1, 10) + 
  ylim(0, 40)
gridExtra::grid.arrange(p1, p2, p3, p4, p9, p5, p6, p7, p8, ncol = 3, nrow = 3)
```
...
<div id="answer">
Type your answer to Queston 5 here and delete this sentence.
