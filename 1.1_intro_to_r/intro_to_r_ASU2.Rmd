...
course and to analyze real data and come to informed conclusions. To clarify which is which: R is the name of the programming language itself and RStudio 
...
```{r label = "load-packages", message = FALSE}
library(dplyr)
library(ggplot2)
library(statsr)
```
...
This command instructs R to load some data. The Arbuthnot data records male and
female christenings (births) in London from 1629 to 1710.    You should see that the workspace 
area in the upper right hand corner of the RStudio window now lists a data set
called `arbuthnot` that has 82 observations on 3 variables. As you interact 
with R, you will create a series of objects.  Sometimes you load them as we have
done here, and sometimes you create them yourself  as the byproduct of a 
...
by clicking on the *x* in the upper left hand corner.
...
<div id="exercise">
**Exercise 1**: What years are included in this dataset? Hint: Take a look at the
year variable in the Data Viewer to answer this question.
</div>
...
lot like functions from mathematics; that is, invoking R commands means supplying
...
place for interacting with your code; however, it is not a good place for 
documenting your work. Working in the console exclusively makes it difficult to 
document your work as you go and to reproduce it later. 
...
in the code chunks provided in the R Markdown (Rmd) document for the lab, and
...
are no longer structured in a table with other variables, they are displayed 
sequentially. Objects that print out in this way are called vectors; 
they represent a set of numbers. R has added numbers in [brackets] along the left
side of the printout to indicate locations within the vector. For example, in the `arbuthnot$boys` vector, 5218  follows [1], indicating that 5218 is the first entry in the vector. And if [43] starts a line, then that would mean the first number on that line would represent the 43^rd^ entry in the vector.

R has some powerful functions for making graphs. We can create a simple plot 
of the number of girls baptized per year with the command

```{r label = "plot-girls-vs-year", fig.cap = "Total number of girls baptized by year"}
ggplot(data = arbuthnot, aes(x = year, y = girls)) +
  geom_point()
```

Before we review the code for this plot, let's summarize the trends we see in Figure \@ref(fig:plot-girls-vs-year).

3. Which of the following best describes the number of girls baptized over the years included in this dataset? 
<ol>
<li> There appears to be no trend in the number of girls baptized from 1629 to 1710. </li>
<li> There is initially an increase in the number of girls baptized, which peaks around 1640. After 1640 there is a decrease in the number of girls baptized, but the number begins to increase again in 1660. Overall the trend is an increase in the number of girls baptized. </li>
<li> There is initially an increase in the number of girls baptized. This number peaks around 1640 and then after 1640 the number of girls baptized decreases. </li>
<li> The number of girls baptized has decreased over time. </li>
<li> There is an initial increase in the number of girls baptized but this number appears to level around 1680 and not change after that time point. </li>
</ol>
...
plotting code in your console, you should see the plot appear under the *Plots* 
tab of the lower right panel of RStudio. Notice that the command above again 
...
object for the plot. Since we want a scatterplot, we use `geom_point`.
...
to examine the sample plots provided on that page and to modify the code 
bit by bit until you create the plot you want.
</div>
...
```{r label = "calc-total-bapt-numbers"}
5218 + 4683
```
...
```{r label = "calc-total-bapt-vars-save"}
arbuthnot <- arbuthnot %>%
...
We can create Figure \@ref(fig:plot-total-vs-year-line), a plot of the total number of baptisms per year with the following command.

```{r label = "plot-total-vs-year-line", fig.cap= "Total number of baptisms per year"}
...
```{r label = "plot-total-vs-year-line-and-point"}
ggplot(data = arbuthnot, aes(x = year, y = total)) +
  geom_line() +
  geom_point() + 
  theme_bw()
...
<div id="exercise">
**Exercise 2**: Create a plot of the proportion of boys born over time. What 
do you see? 
</div>



```{r label = "plot-proportion-of-boys-over-time", fig.cap="Proportion of boys born over time"}
# Type your code for Exercise 2 here, and Knit.

```

<div id="answer">
Type your answer for Exercise 2 and delete this sentence.
...
```{r label = "boys-more-than-girls"}
arbuthnot <- arbuthnot %>%
  mutate(more_boys = boys > girls)
DT::datatable(arbuthnot)
```
...
frame have values are numerical (the year, the numbers of boys and girls). Here, 
we've asked R to create *logical* data, data where the values are either `TRUE` 
or `FALSE`. In general, data analysis will involve many different data 
...
but for present day birth records in the United States. Load the 
present day data with the following command.

```{r label = "load-present-data"}
data(present)
```
...
<div id="exercise">
**Exercise 3**: What years are included in this dataset? **Hint:** Use the `range` 
...
```{r label = "prop-boys-over-time", fig.cap = "Proportion of boys born over time for the `present` data set"}
...
```{r label = "prop-boy-girl-over-time", fig.cap = "Ratio of boys to girls over time"}
...
names. Or to arrange the data in a descending order with new function: `descr` (for 
descending order).
<ol>
<li> 1940 </li>
<li> 1957 </li>
...
```{r label = "most-total-births"}
# Type your code for Question 8 here, and Knit.

```

<div id="answer">
Type your answer for Question 8 here and delete this sentence.
...
- The following cheat sheets may come in handy throughout the course. Note that some 
of the code on these cheat sheets may be too advanced for this course; however, the 
...
working assignments in the relevant courses at [DataCamp](https://www.datacamp.com/courses).
