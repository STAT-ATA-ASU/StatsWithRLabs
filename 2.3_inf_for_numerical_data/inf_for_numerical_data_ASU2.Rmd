...
mothers and the births of their children. We will work with a random sample of 
observations from this data set.

Load the `nc` data set into the workspace.
...
```{r label = "MP", fig.cap = "Weight (lbs.) gained during pregnancy"}
...
The distribution of pounds gained during pregnancy in Figure \@ref(fig:MP) is unimodal and skewed to the right.  The median is `r median(nc$gained, na.rm = TRUE)` pounds while the interquartile range of the is `r IQR(nc$gained, na.rm = TRUE)` pounds.
...
Next, consider the possible relationship between a mother's smoking habits and the 
...
```{r label = "habit-weight-box"}
# Type your code for Question 3 here, and Knit.

```
Figure \@ref(fig:rmnas) removes the missing values and uses the $x-$ axis as the response.

```{r, label = "rmnas", fig.cap = "Side-by-side boxplots of `weight` for smoking and nonsmoking mothers"}
ND <- nc %>%
  filter(!is.na(habit)) 
ggplot(data = ND, aes(x = habit, y = weight, fill = habit)) + 
  geom_boxplot() + 
  guides(fill = FALSE) + 
  coord_flip() + 
  theme_bw()
...
also compare the means of the distributions using the following to group the 
data by the `habit` variable, and then calculate the mean `weight` in these 
groups using the `mean` function.

```{r label = "by-means"}
nc %>%
  group_by(habit) %>%
  summarise(mean_weight = mean(weight))
# Another approach is to use tapply()
...
<div id="exercise">
**Exercise 1**: Are all conditions necessary for inference satisfied? Comment on each. You can compute the group sizes using the same `by` command above but replacing `mean(weight)` with `n()`.
</div>

```{r, label = "satisfiedornot"}
...
```{r label = "inf-weight-habit-ht"}
inference(y = weight, x = habit, data = nc, statistic = "mean", type = "ht", null = 0, 
          alternative = "twosided", method = "theoretical")
```

Let's pause for a moment to go through the arguments of this custom function. 
The first argument is `y`, which is the response variable in which we are 
interested: `weight`. The second argument is the explanatory variable, 
`x`, which is the variable that splits the data into two groups, smokers and 
non-smokers: `habit`. The third argument, `data`, is the data frame in which these
variables are stored. Next is `statistic`, which is the sample statistic
...
<div id="exercise">
**Exercise 2**: What is the conclusion of the hypothesis test?
</div>
...
<div id="answer">
Type your answer for Question 6 here and delete this sentence - interpret the CI.


</div>

<div id="exercise">
**Exercise 3**: Calculate a new confidence interval for the same parameter at the 90% confidence level. Comment on the width of this interval versus the one obtained in the the previous exercise.
</div>
...
<div id="answer">
Type your answer for Exercise 3 here and delete this sentence - interpret the CI.


</div>

<div id="exercise">
**Exercise 4**: Conduct a hypothesis test evaluating whether the average weight gained by younger mothers is different from the average weight gained by mature mothers.
</div>

```{r label = "gained-mature-ht"}
...
<div id="exercise">
**Exercise 5**: Pick a pair of variables: one numerical (response) and one categorical 
...
and analyzing the data to answer this question is basically what you will need to do for
your project as well.)
</div>

```{r label = "pick-your-own"}
